/*
	Description:
	This file is to create the testbed for the HTB feature, It can be used in the following cases:
	1, With the kubenertes starts, flannel distribute the subnet on different nodes (VM), on each node, run this code to generate the specified IP address Pod, and then add a default route on each Pod.
	2. A pure docker + flannel + etcd cases, run this code to generate many Pods with specified IP address and default route
*/
