...
func set_pod_br_inbound_bandwidth_class_and_filter(br_name string, pod_qos map[string]qos_para,
	...
	// here create an array to store all the "key" (container IP address)
	ip_keys := make([]string, 0, len(pod_qos)-2)
	for k := range pod_qos {
		if k != "all" && k != "default" {
			keys = append(keys, k)
		}
	...
	// then we seperate the pod_qos into several groups, and run parallel to config
	num_routine := 5
